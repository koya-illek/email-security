...
// outlive: v8 stops 24h edge-cached rows — including those generated by the
// long-stale production deploy with its pre-iteration-4 analysis engine — from
// serving superseded evidence after the next deploy.
const CACHE_VERSION = 'v9-dmarc-sp-spf-redirect';
...
async function checkPTR(domain, mxRecords = [], budget = null) {
  ...
  for (const mxHost of hosts) {
    ...
    for (const [ip, normalizedIp] of normalizedAddresses) {
      ...
      const matches = Boolean(ptr && forward.some(candidate =>
        candidate === ip ||
        (ipaddr.isValid(candidate) && ipaddr.parse(candidate).toString() === normalizedIp)
      ));
      observations.push({ mxHost, ip, ptr, matches });
    }
  }
