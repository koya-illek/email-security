...
let { spf } = require('mailauth/lib/spf');

const suiteFile = fs.readFileSync(__dirname + '/fixtures/rfc7208-tests.yml', 'utf8');
            ...
            for (let test of Object.keys(file.tests)) {
                ...
                it(test, async () => {
                    ...
                    if (Array.isArray(testdata.result)) {
                        assert.ok(
                            testdata.result.includes(result?.status?.result),
                            `${test}: expected one of ${testdata.result.join(', ')}, got ${result?.status?.result}`
                        );
                    } else {
                        assert.equal(result?.status?.result, testdata.result, test);
                    }
                });
            }
        });
